...
function AdminDashboard() {
  ...
  // ✅ All functions wrapped in useCallback (ESLint FIX)
  ...
  const updateOrderStatus = async (id, newStatus) => {
    try {
      await axios.patch(
        `${API}/admin/orders/${id}/status?status=${newStatus}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success('Order status updated');
  ...
  const updateUserRole = async (id, newRole) => {
    try {
      await axios.patch(
        `${API}/admin/users/${id}/role?new_role=${newRole}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success('User role updated');
    ...
    <div className="admin-container">
      ...
      {activeTab === 'stats' && (
        stats ? (
          <>
            <div className="stats-cards">
              <div className="stat-card">👥 {stats.total_users}</div>
              <div className="stat-card">📦 {stats.total_products}</div>
              <div className="stat-card">📋 {stats.total_orders}</div>
              <div className="stat-card">💰 ₹{stats.monthly_income}</div>
            </div>
